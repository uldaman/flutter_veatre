...
class Mnemonic {
  static final int seedIterations = 2048;
  static final int seedKeySize = 64;
  ...
  static Future<String> generateMnemonic(Uint8List initialEntropy) async {
    if (wordList == null) {
      wordList = await populateWordList();
    }
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      mnemonicBuilder.write(wordList[index]);
...
// Generate the master seed from the mnemonic and the passphrase [The passphrase is optional]
  static Uint8List generateMasterSeed(String mnemonic, String passphrase) {
    ...
    derivator.init(
        new Pbkdf2Parameters(utf8.encode(salt), seedIterations, seedKeySize));
  ...
  static Future<List<String>> populateWordList() async {
